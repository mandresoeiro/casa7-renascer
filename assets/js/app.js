...
if(welcomeBanner && c.bannerInicial?.imagem){
 ...
 let dismissed=false;
 try{dismissed=sessionStorage.getItem('casa7-banner-oracao-2026-v3')==='fechado';}catch(error){}
 if(bannerImage){bannerImage.src=c.bannerInicial.imagem;bannerImage.alt=c.bannerInicial.descricao||'Aviso da Casa 7';}
 const closeBanner=()=>{try{sessionStorage.setItem('casa7-banner-oracao-2026-v3','fechado');}catch(error){}welcomeBanner.classList.remove('is-visible');document.body.classList.remove('banner-open');setTimeout(()=>{welcomeBanner.hidden=true;},180);};
...
if(miniPlayer && audio && tracks.length){
 ...
 const playlistOpen=$('[data-playlist-open]');
 const playlistClose=$('[data-playlist-close]');
 let currentTrack=0;
 ...
 const trackButtons=tracks.map((track,index)=>{if(!playlistTracks)return null;const button=document.createElement('button');button.className='playlist-track';button.type='button';button.dataset.trackIndex=String(index);const number=document.createElement('span');number.className='playlist-track-number';number.textContent=String(index+1).padStart(2,'0');const copy=document.createElement('span');copy.className='playlist-track-copy';const name=document.createElement('strong');name.textContent=track.titulo||'Faixa '+(index+1);const by=document.createElement('span');by.textContent=track.artista||'Casa 7';copy.append(name,by);button.append(number,copy);playlistTracks.append(button);return button;});
 const updateTrackList=()=>trackButtons.forEach((button,index)=>{if(!button)return;button.classList.toggle('is-current',index===currentTrack);if(index===currentTrack)button.setAttribute('aria-current','true');else button.removeAttribute('aria-current');});
 const loadTrack=index=>{currentTrack=(index+tracks.length)%tracks.length;const track=tracks[currentTrack];audio.src=track.arquivo;audio.load();if(title)title.textContent=track.titulo||'Faixa '+(currentTrack+1);if(artist)artist.textContent=String(currentTrack+1).padStart(2,'0')+' de '+tracks.length+' · '+(track.artista||'Casa 7');if(progress)progress.value='0';if(currentTime)currentTime.textContent='0:00';if(duration)duration.textContent='0:00';updateTrackList();};
 ...
 const setPlaylistOpen=open=>{if(!playlistPanel)return;playlistPanel.hidden=!open;playlistOpen?.setAttribute('aria-expanded',String(open));if(open)playlistPanel.scrollIntoView({behavior:'smooth',block:'nearest'});};
 loadTrack(0);
 ...
 playlistOpen?.addEventListener('click',()=>setPlaylistOpen(playlistPanel?.hidden!==false));
 playlistClose?.addEventListener('click',()=>setPlaylistOpen(false));
 trackButtons.forEach((button,index)=>button?.addEventListener('click',()=>{loadTrack(index);playCurrent();}));
